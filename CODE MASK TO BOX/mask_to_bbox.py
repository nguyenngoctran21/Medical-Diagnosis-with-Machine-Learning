...
def write_bbox_info_to_file(file_path, bboxes, width, height):
    with open(file_path, 'w') as f:
        for bbox in bboxes:
            ...
            x_max = bbox[2] / width
            y_max = bbox[3] / height
            # Format the coordinates as required by Labelbox
            formatted_bbox = f"polyp {x_min} {y_min} {x_max} {y_max}"
            f.write(formatted_bbox + '\n')
...
if __name__ == "__main__":
    ...
    for x, y in tqdm(zip(images, masks), total=len(images)):
        ...
        """ Saving bounding box info """
        height, width, _ = x.shape
        write_bbox_info_to_file(f"results/{name}.txt", bboxes, width, height)
