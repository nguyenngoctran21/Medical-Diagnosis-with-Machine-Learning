...
parser.add_argument('--n_neighbors', type=int, help='Specify the value of n_neighbors')
parser.add_argument('--weights', type=str, help='Specify the weights')
parser.add_argument('--p', type=int, help='Specify the p')
...
n_neighbors_value = args.n_neighbors
weights_value = args.weights
p_value = args.p
# In giá trị của C và solver
print(f'Giá trị algorithm nhận từ dòng lệnh: {algorithm_value}')
print(f'Giá trị neighbors nhận từ dòng lệnh: {n_neighbors_value}')
print(f'Giá trị weights nhận từ dòng lệnh: {weights_value}')
print(f'Giá trị p nhận từ dòng lệnh: {p_value}')

# Hyperparameter tuning for KNeigh borsClassifier
log_reg_grid = {
    'algorithm': [algorithm_value],
    'n_neighbors': [n_neighbors_value],  # Chọn giá trị K (số lân cận) mong muốn
    'weights': [weights_value],  # Cân nhắc việc sử dụng trọng số đều hoặc dựa vào khoảng cách
    'p': [p_value],  # Lựa chọn khoảng cách (1: Manhattan, 2: Euclidean)
}
   



gs_log_reg = GridSearchCV(KNeighborsClassifier(), log_reg_grid, cv=2, verbose=True)
...
cv_accuracy = np.mean(cross_val_score(gs_log_reg, X, y, scoring='accuracy', cv=1))
cv_precision = np.mean(cross_val_score(gs_log_reg, X, y, scoring='precision', cv=1))
cv_recall = np.mean(cross_val_score(gs_log_reg, X, y, scoring='recall', cv=1))
cv_f1 = np.mean(cross_val_score(gs_log_reg, X, y, scoring='f1', cv=1))
cv_auc_roc = np.mean(cross_val_score(gs_log_reg, X, y, scoring='roc_auc', cv=1))
...
feature_df.T.plot.bar(title='Feature Importance of KNeighbors Classifier', legend=False);
...
with open(output_file_path, 'w') as file:
    ...
    file.write(f"Siêu tham số n_neighbors: {args.n_neighbors}\n")
